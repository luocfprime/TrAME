...
class GaussianEditor(BaseLift3DSystem):
    ...
    class Config(BaseLift3DSystem.Config):
        ...
        edit_begin_step: int = 0  # after which will start SD editing
        edit_until_step: int = 10000  # after which will only perform GS optimization with no SD editing
    ...
    def update_mask(self, save_name="mask") -> None:
        ...
        if not os.path.exists(gs_mask_path) or self.cfg.cache_overwrite:
            ...
            for id in tqdm(self.view_list):
                ...
                self.gaussian.apply_weights(cur_cam, weights, weights_cnt, mask)  # mask: (bsz, H, W), weights, weights_cnt: (N_gs, 1)

            weights /= weights_cnt + 1e-7

            selected_mask = weights > self.cfg.mask_thres
            selected_mask = selected_mask[:, 0]  # (num_gaussians, 1) -> (num_gaussians)
        ...
        if not selected_mask.any():
            threestudio.warn("selected_mask is empty, reset to all True to use all gaussians.")
            selected_mask.fill_(True)
    ...
    def forward(self, batch: Dict[str, Any], renderbackground=None, local=False) -> Dict[str, Any]:
        ...
        for id, cam in enumerate(batch["camera"]):
            ...
            semantic_map = torch.norm(semantic_map, dim=0)  # 3 H W -> H W
            semantic_map_soft = torch.sigmoid(semantic_map)
            ...
            soft_masks.append(semantic_map_soft)
        ...
        soft_masks = torch.stack(soft_masks, dim=0)

        render_pkg["semantic"] = semantics
        render_pkg["masks"] = masks
        render_pkg["soft_masks"] = soft_masks
    ...
    def test_step(self, batch, batch_idx):
        ...
        # if not hasattr(self, "rendered_frames"):
        #     self.rendered_frames = {}
        # self.rendered_frames[int(batch['index'][0])] = out["comp_rgb"][0]
    ...
    def on_test_epoch_end(self):
        ...
        # rendered_frames = sorted(self.rendered_frames.items(), key=lambda item: item[0])
        # rendered_frames = torch.stack([frame for _, frame in rendered_frames], dim=0)
        # rendered_frames = rearrange(rendered_frames, "views h w c -> c h (views w)")
        # ToPILImage()(rendered_frames).save(self.get_save_path(f"rendered_frames.png"))
    ...
    def configure_optimizers(self):
        ...
        opt = OptimizationParams(self.parser, self.trainer.max_steps, self.cfg.gs_lr_scaler,
                                 self.cfg.gs_final_lr_scaler, self.cfg.color_lr_scaler,
                                 self.cfg.opacity_lr_scaler, self.cfg.scaling_lr_scaler, self.cfg.rotation_lr_scaler, self.cfg.color_rest_lr_scaler)
