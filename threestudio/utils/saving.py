...
class SaverMixin:
    ...
    def _save_rgb_image(
        ...
        # if SaverMixin is used in a PyTorch Lightning module and tensorboard is used as logger
        if name and hasattr(self, "trainer") and isinstance(self.trainer.logger, TensorBoardLogger):
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            self.trainer.logger.experiment.add_image(
                name, img, global_step=step, dataformats="HWC"
            )
