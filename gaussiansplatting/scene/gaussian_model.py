...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            },
            {
                ...
                "lr": training_args.feature_rest_lr,
                "name": "f_rest",
            },
            {
    ...
    def reset_opacity_with_mask(self, reset_val=0.01):
        # 1. reset opacity
        self._opacity.data[self.mask] = inverse_sigmoid(
            torch.min(
                self.get_opacity[self.mask], torch.ones_like(self.get_opacity[self.mask]) * reset_val
            )
        )
        # 2. reset related optimizer state
        for group in self.optimizer.param_groups:
            if group["name"] == "opacity":
                stored_state = self.optimizer.state.get(group["params"][0], None)
                stored_state["exp_avg"][self.mask] = 0
                stored_state["exp_avg_sq"][self.mask] = 0
                break
    ...
    def save_init_gaussians(self):
        """Run this after configure optimizer"""
        self._xyz_init = self._xyz.data.detach().clone()
        self._features_dc_init = self._features_dc.data.detach().clone()
        self._features_rest_init = self._features_rest.data.detach().clone()
        self._scaling_init = self._scaling.data.detach().clone()
        self._rotation_init = self._rotation.detach().data.clone()
        self._opacity_init = self._opacity.detach().data.clone()

        self.max_radii2D_init = self.max_radii2D.detach().clone()
        self.xyz_gradient_accum_init = self.xyz_gradient_accum.detach().clone()
        self.denom_init = self.denom.detach().clone()

        self.mask_init = self.mask.detach().clone()
        self._generation_init = self._generation.detach().clone()

    def reset_init_gaussians(self):
        self._xyz.data = self._xyz_init.detach().clone()
        self._features_dc.data = self._features_dc_init.detach().clone()
        self._features_rest.data = self._features_rest_init.detach().clone()
        self._scaling.data = self._scaling_init.detach().clone()
        self._rotation.data = self._rotation_init.detach().clone()
        self._opacity.data = self._opacity_init.detach().clone()

        self.max_radii2D = self.max_radii2D_init.detach().clone()
        self.xyz_gradient_accum = self.xyz_gradient_accum_init.detach().clone()
        self.denom = self.denom_init.detach().clone()

        self.mask = self.mask_init.detach().clone()
        self._generation = self._generation_init.detach().clone()

        # reset optimizer states
        param_keys = ["xyz", "f_dc", "f_rest", "opacity", "scaling", "rotation"]
        for group in self.optimizer.param_groups:
            if group["name"] in param_keys:
                stored_state = self.optimizer.state.get(group["params"][0], None)
                if stored_state is None:
                    continue
                device = stored_state["exp_avg"].device
                dtype = stored_state["exp_avg"].dtype
                stored_state["exp_avg"] = torch.zeros_like(group["params"][0], dtype=dtype, device=device)
                stored_state["exp_avg_sq"] = torch.zeros_like(group["params"][0], dtype=dtype, device=device)

        self.remove_grad_mask()
        self.apply_grad_mask(self.mask)
        self.update_anchor()  # reset anchor
    ...
    def densify_and_prune(
            self, max_grad, max_densify_percent, min_opacity, extent, max_screen_size, min_scale=None
    ):
        ...
        print(f"Generation_{self.generation_num}: before: {before} - clone: {clone} - split: {split} ")
        ...
        if min_scale is not None:
            ...
            prune_mask = torch.logical_or(
                prune_mask, self.get_scaling.max(dim=1).values < min_scale
            )
        ...
        return (
            before,
            clone - before,  # number of clone
            split - clone,  # number of split
            prune - split,  # number of prune
        )
    ...
    def all_params_requires_grad_(self, requires_grad):
        fields = ["_xyz", "_features_dc", "_features_rest", "_opacity", "_scaling"]

        for field in fields:
            this_field = getattr(self, field)
            this_field.requires_grad_(requires_grad)
